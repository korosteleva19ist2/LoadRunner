...
/*Correlation comment - Do not change!  Original value='137655.668043742HAiAzzDptcftctiDpQiAff' Name ='CorrelationParameter' Type ='Manual'*/
...
lr_end_transaction("LogIn", LR_AUTO);
	...
	if(randNumber==1)
	{
		//web_reg_find("Text=A {seat_Type} Class ticket from {depart_City} to {arrive_City}.</u>",LAST);
	...
	//ПОДХОДЯТ ЛИ ЭТИ 2 ПРОВЕРКИ?
	//проверка соответствия указанного класса перелета
	if(randNumber==1)
	...
	//проверка добавления рейса в список
	...
	lr_end_transaction("UC2_RegistrationReservAddView", LR_AUTO);
